...
def montar_tela_principal(container):
    ...
    btn_novo_cad = tk.Button(container, text="NOVO CADASTRO", width=25, height=2, 
                             command=lambda: categoria.abrir_cadastro(container), bg="#e1e1e1")
    btn_novo_cad.pack(pady=5)
 
    btn_cons_lista = tk.Button(container, text="CONSULTAR LISTA", width=25, height=2,
                               command=lambda: categoria.abrir_consulta(container), bg="#e1e1e1")
    btn_cons_lista.pack(pady=5)
 
    btn_cad_livro = tk.Button(container, text="NOVO CADASTRO LIVRO", width=25, height=2, 
